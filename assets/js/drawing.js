...
class GridRenderer {
    constructor(canvas, grid) {
        ...
        this.animationSpeed = 150; // ms between frames
    ...
    drawGrid() {
        ...
        if ((!grassTilesAvailable && grassTileset) || (!waterTilesAvailable && waterTileset)) {
    ...
    // Add this new method to handle animation
    startAnimation() {
        const animate = (timestamp) => {
            // Update animation frame at specified speed
            if (timestamp - this.lastFrameTime > this.animationSpeed) {
                this.lastFrameTime = timestamp;
                this.animationFrame = (this.animationFrame + 1) % 4; // 4 water tiles
                this.drawGrid();
            }
